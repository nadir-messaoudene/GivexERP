    ...
    def create_filter(self):
        ...
        if self.env.user.ringcentral_access_token:
            headers['authorization'] = 'Bearer ' + \
                self.env.user.ringcentral_access_token
        params = [
            ('view', 'Detailed'),
            ('page', '1'),
            ('perPage', '10000'),
        ...
        response = requests.get(
            'https://platform.devtest.ringcentral.com/restapi/v1.0/account/~/call-log', headers=headers, params= tuple(params))
        rec_dict = response.json()
        if not rec_dict.get('errorCode'):
            self.env['crm.phonecall'].synch_data(rec_dict.get('records'))
        else:
            raise UserError(_('Please login in a ringcentral.'))
        params = [
            ('page', '1'),
            ('perPage', '1000'),
        ]
        params += rec_li
        response = requests.get('https://platform.devtest.ringcentral.com/restapi/v1.0/account/~/extension/~/message-store', headers=headers, params=tuple(params))
        rec_dict = response.json()
        if not rec_dict.get('errorCode'):
