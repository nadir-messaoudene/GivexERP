odoo.define('ringcentral.ringcentralPanel', function(require) {
    ...
    var ringcentralPanel = Widget.extend({
        ...
        start: function() {
            ...
            ajax.jsonRpc('/ringcentral_credentials', 'call', {
                ...
                if (data['ringcentral_server'] && data['ringcentral_app_key'] && data['ringcentral_app_secret'] && data['ringcentral_redirect_uri'] && data['ringcentral_app_host'] && data['ringcentral_app_port']) {
                    ...
                    self.ringcentral_redirect_uri = data['ringcentral_app_secret'];
            ...
            return rpc.query({
                    ...
                    args: [
                        ...
                        ["phone", "name", "image_128", "email", "mobile"]
                    ],
                    context: session.context
                })
                .then(function(partner_ids) {
                    self.contacts = partner_ids;
                    ...
                    for (var i = 0; i < partner_ids.length; i++) {
                        if (partner_ids[i].phone){
                            option = new Option(partner_ids[i].name + ' <' + partner_ids[i].phone + '>', partner_ids[i].phone);
                        }else{
                            option = new Option(partner_ids[i].name + ' <' + partner_ids[i].mobile + '>', partner_ids[i].mobile);
                        }
                        select.append($(option));
                        self.contacts_partner[partner_ids[i].id] = partner_ids[i];
                        self.partner_search_string += self._partner_search_string(partner_ids[i]);
                    }
                }).then(function() {
        ...
        onInvite: function(session) {
            ...
            self._rpc({
                ...
                $modal_incoming_call.find('tr').on('click', function(ev) {
                   if ($(ev.currentTarget).attr('data-id')) {
                        self.do_action({
                            'view_type': 'form',
                            'views': [[false, 'form']],
                            'res_model': 'res.partner',
                            'res_id': parseInt($(ev.currentTarget).attr('data-id')),
        ...
        sub_onaccepted: function(self, session, to_number, from_number, type, caller_name, caller_number, data={}) {
            ...
            session.on('terminated', function() {
                self.platform.get('/restapi/v1.0/account/~/extension/~/active-call').then(function(results) {
                    ...
                    _.each(record['records'], function(result) {
                        rpc.query({
                            ...
                            if (data.length <= 0 && result && result.from && (result.from.phoneNumber == to_number && result.result == 'Accepted' || result.to.phoneNumber == to_number && result.result == 'Call connected')) {
                                ...
                                if (result.recording) {
                                    ...
                                    str = url[0] + '/mobile/media?cmd=downloadMessage&msgid=' + result.recording.id + '&useName=true&time=' + '1554700788480' + '&msgExt=&msgNum=' + result.from.phoneNumber + '&msgDir=' + result.direction + '&msgRecType=' + rec_type + '&msgRecId=' + result.recording.id + '&type=1&download=1&saveMsg=&file=.mp3'
                                }
                                    var data_li = []
                                if (result.legs) {
                                    _.each(result.legs, function(leg){
                                        var val = {'name': leg.action, 'call_type': leg.direction, 'leg_type' : leg.legType, 'from_number' : leg.from.phoneNumber, 'to_number' : leg.to.phoneNumber}
            ...
            session.on('bye', function() {
                ...
                // for (var i = 0; i < 2; i++) {
                self.platform.get('/restapi/v1.0/account/~/extension/~/active-calls').then(function(results) {
                    ...
                    for (var i = 0; i < self.contacts.length; i++) {
                        for (var j = 0; j < calls.length; j++) {
                            ...
                            if (self.contacts[i].phone == calls[j]["to"]["phoneNumber"] || self.contacts[i].mobile == calls[j]["to"]["phoneNumber"]) {
                                calls[j]["to"]["name"] = self.capitalizeFirstLetter(self.contacts[i].name)
                            } else {
                                if (calls[j]["to"]["name"]) {
                            ...
                            if (messages[j]["direction"] == "Outbound") {
                                for (var k = 0; k < messages[j]["to"].length; k++) {
                                    if (self.contacts[i].phone == messages[j]["to"][k]["phoneNumber"] || self.contacts[i].mobile == messages[j]["to"][k]["phoneNumber"]) {
                                        messages[j]["to"][k]["partner_name"] = self.capitalizeFirstLetter(self.contacts[i].name)
                                    }
                                }
                                ...
                                if (self.contacts[i].phone == messages[j]["from"]["phoneNumber"] || self.contacts[i].mobile == messages[j]["from"]["phoneNumber"]) {
                                    messages[j]["from"]['partner_name'] = self.capitalizeFirstLetter(self.contacts[i].name)
                                }
                            }
                        }
                    }
                    ...
                    _.each(messages, function(message) {
                        if (self.messages.indexOf(message.id) == -1) {
                            ...
                            Notification.requestPermission().then(function(result) {
                                new Notification('new Message Arrive')
                            });
