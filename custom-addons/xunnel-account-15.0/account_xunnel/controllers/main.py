        ...
        if event == 'refresh':
            for _, journal_data in data.items():
                ...
                if not online_identifier:
                    continue
                journal = request.env['account.online.account'].sudo().search(
                    [('online_identifier', '=', online_identifier)], limit=1)
                journal._retrieve_transactions(forced_params=journal_data)
        else:
            request.env['res.company'].sudo()._sync_xunnel_providers(provider)
