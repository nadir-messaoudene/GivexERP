...
class BankAccRecStatement(models.Model):
    ...
    def create(self, vals):
        record = super(BankAccRecStatement, self).create(vals)
        for rec in record:
            lines = rec.credit_move_line_ids + rec.debit_move_line_ids
            for line in lines:
                if line.move_line_id:
                    line.move_line_id.write({'cleared_bank_account': True})
        return record
    ...
    def write(self, vals):
        ...
        for record in self:
            lines = record.credit_move_line_ids + record.debit_move_line_ids
            for line in lines:
                if line.move_line_id:
                    line.move_line_id.write({'cleared_bank_account': True})
        return res
    ...
    def unlink(self):
        ...
        for statement in self:
            ...
            for line in statement_lines:
                if line.move_line_id:
                    line.move_line_id.write({'cleared_bank_account': False})
    ...
    def action_review(self):
        ...
        for rec in self:
            ...
            lines = rec.credit_move_line_ids + rec.debit_move_line_ids
            s_line = lines.filtered(lambda r: r.cleared_bank_account)
            if len(s_line.ids) == 0:
                raise UserError(
                    _("Please select atleast one statement line for Review!"))
    ...
    def action_process(self):
        ...
        for statement in self:
            statement.clear_bank_statement_line()
            statement_lines = statement.credit_move_line_ids + \
                statement.debit_move_line_ids
    ...
    def _compute_balance(self):
        ...
        for statement in self:
            ...
            for line in statement.credit_move_line_ids:
                sum_of_credits += line.cleared_bank_account and round(line.amount, account_precision) or 0.0
                sum_of_credits_lines += line.cleared_bank_account and 1.0 or 0.0
            for line in statement.debit_move_line_ids:
                sum_of_debits += line.cleared_bank_account and round(line.amount, account_precision) or 0.0
                sum_of_debits_lines += line.cleared_bank_account and 1.0 or 0.0
            cleared_balance = round((sum_of_debits - sum_of_credits), account_precision)
            difference = round((statement.ending_balance - statement.starting_balance) - cleared_balance, account_precision)
    ...
    def _get_move_line_write(self, line, multi_currency):
        ...
        res = {'ref': line.ref or line.name or "Line",
               'date': line.date,
               'partner_id': line.partner_id.id,
               'currency_id': line.currency_id.id,
               'amount': abs(amount),
               'name': line.name or line.ref or "Statement Line",
    ...
    def _get_exits_move_line(self, mv_line_rec):
        res = {}
        statement_lines = self.credit_move_line_ids + self.debit_move_line_ids
        for statement_line in statement_lines:
            if statement_line.move_line_id and statement_line.move_line_id.id == mv_line_rec.id:
                res.update({
                    'cleared_bank_account': statement_line.cleared_bank_account,
                    'ref': statement_line.ref or mv_line_rec.name or "Line",
                    ...
                    'name': statement_line.name or mv_line_rec.name or 'Line',
    ...
    def refresh_record(self):
        ...
        for obj in self:
            ...
            if account_curr_id and cmpny_curr_id and account_curr_id.id != cmpny_curr_id.id:
                to_write['multi_currency'] = True
            move_line_ids = [
                line.move_line_id.id for line in obj.credit_move_line_ids + obj.debit_move_line_ids if line.move_line_id]
            domain = [('id', 'not in', move_line_ids),
            ...
            for line in lines:
                res = (0, 0, self._get_move_line_write(
                    line, to_write['multi_currency']))
                if line.credit:
    ...
    def onchange_account_id(self):
        ...
        if self.account_id:
            ...
            if acc_curr_id and cmpny_curr_id and \
                ...
                multi_currency = True
            last_rec_id = self.search([
                ('account_id', '=', self.account_id.id),
                ('ending_date', '<=', self.ending_date),
                ('state', '=', 'done'),
            ], limit=1, order='id desc')
            if last_rec_id and last_rec_id.id:
                self.starting_balance = last_rec_id.ending_balance
            for statement in self:
                statement_lines = statement.credit_move_line_ids + \
                    statement.debit_move_line_ids
            ...
            for line in line_ids:
                if line.id not in move_line_ids:
                    res = self._get_move_line_write(line, multi_currency)
                else:
                    res = self._get_exits_move_line(line)
                if res.get('type') == 'cr':
                    credit_move_line_ids += [(0, 0, res)]
                else:
                    debit_move_line_ids += [(0, 0, res)]
        self.debit_move_line_ids = debit_move_line_ids
        self.credit_move_line_ids = credit_move_line_ids

#    def is_b_a_r_s_state_done(self, line):
#        for rec in self:
#            statement_line_ids = self.env['bank.acc.rec.statement.line'].search([
#                ('move_line_id', '=', line)])
...
#            return True
    ...
    account_id = fields.Many2one(
        ...
        states={'done': [('readonly', True)],
                'to_be_reviewed': [('readonly', True)]},
        domain="[('company_id', '=', company_id)]",
        help="The Bank/Gl Account that is being reconciled.")
    ending_date = fields.Date(
        'Ending Date', required=True, default=time.strftime('%Y-%m-%d'),
        states={'done': [('readonly', True)],
                'to_be_reviewed': [('readonly', True)]}, tracking=True,
        help="The ending date of your bank statement.")
    starting_balance = fields.Float(
        'Starting Balance', required=True, digits='Account', tracking=True,
        help="The Starting Balance on your bank statement.",
        states={'done': [('readonly', True)]})
    ending_balance = fields.Float(
        'Ending Balance', required=True, digits='Account',
        help="The Ending Balance on your bank statement.", tracking=True,
        states={'done': [('readonly', True)]})
    ...
    credit_move_line_ids = fields.One2many(
        'bank.acc.rec.statement.line', 'c_statement_id', 'Credits',
    ...
    state = fields.Selection([
        ('draft', 'Draft'), ('to_be_reviewed', 'Ready for Review'),
        ('done', 'Done'), ('cancel', 'Cancel')], 'State', tracking=True,
...
class BankAccRecStatementLine(models.Model):
    ...
    def unlink(self):
        ...
        if move_line_ids:
            ...
            account_move_line_brw.write({'cleared_bank_account': False,
                                         'bank_acc_rec_statement_id': False})
