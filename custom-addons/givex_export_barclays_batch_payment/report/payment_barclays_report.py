...
class BatchPaymentCSV(models.AbstractModel):
    _name = "report.report_csv.batch_payment_csv"
    ...
    def generate_csv_report(self, writer, data, batch_payments):
        ...
        for batch_payment in batch_payments.sudo():
            for payment in batch_payment.payment_ids:
                ...
                writer.writerow(
                    {
                        "Destination Sort Code": payment.partner_bank_account_id.sort_code
                        and int(payment.partner_bank_account_id.sort_code[:6])
                        or "",
                        ...
                        or "",
                        "Debit Sort Code": payment.journal_id.bank_account_id.sort_code
                        and int(payment.journal_id.bank_account_id.sort_code[:6])
                        or "",
                        ...
                        or "",
                        "Payment Reference": payment.name and payment.name[:18] or "",
...
class BatchPaymentXLSX(models.AbstractModel):
    _name = "report.report_xlsx.batch_payment_xlsx"
        ...
        for batch_payment in batch_payments.sudo():
            ...
            for payment in batch_payment.payment_ids:
                ...
                sheet.write(
                    payment_row,
                    payment_col + 0,
                    payment.partner_bank_account_id.sort_code
                    and int(payment.partner_bank_account_id.sort_code[:6])
                    or "",
                ...
                sheet.write(
                    payment_row,
                    payment_col + 4,
                    payment.journal_id.bank_account_id.sort_code
                    and int(payment.journal_id.bank_account_id.sort_code[:6])
                    or "",
                    numbersformat_locked,
                )
                ...
                sheet.write(
                    payment_row,
                    payment_col + 9,
                    payment.name and payment.name[:18] or "",
                    locked,
                )
                sheet.write(
                    payment_row,
