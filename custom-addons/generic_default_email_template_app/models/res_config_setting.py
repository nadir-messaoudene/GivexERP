...
class Company(models.Model):
	...
	sale_quatation_template = fields.Many2one('mail.template', string="Template")
	sale_order_template = fields.Many2one('mail.template', string="Template")
	purchase_quatation_template = fields.Many2one('mail.template', string="Template")
	purchase_order_template = fields.Many2one('mail.template', string="Template")
	invoice_template = fields.Many2one('mail.template', string="Template")
	
class ResConfigSettings(models.TransientModel):
	_inherit = 'res.config.settings'

	sale_quatation_template = fields.Many2one(related='company_id.sale_quatation_template', string="Template", readonly = False)
	sale_order_template = fields.Many2one(related='company_id.sale_order_template', string="Template", readonly = False)
	purchase_quatation_template = fields.Many2one(related='company_id.purchase_quatation_template', string="Template", readonly = False)
	purchase_order_template = fields.Many2one(related='company_id.purchase_order_template', string="Template", readonly = False)
	invoice_template = fields.Many2one(related='company_id.invoice_template', string="Template", readonly = False)
...
class InheritMessase(models.TransientModel):
	...
	def default_get(self, fields_list):
		attachment_id = self._context.get('active_ids')
			...
			if res['model'] == str('sale.order'):
				...
				if sale_id.state == 'draft':
					if 'template_id' in res:
						if company.sale_quatation_template.model_id.model == 'sale.order':
							...
							raise Warning(_("You must select email template in general settings only for Sale Order !"))
					else:
						if company.sale_quatation_template.model_id.model == 'sale.order':
							res.update({'template_id' : company.sale_quatation_template.id})
						else:
				...
				if purchase_id.state == 'draft':
					if 'template_id' in res:
						if company.purchase_quatation_template.model_id.model == 'purchase.order':
							...
							raise Warning(_("You must select email template in general settings only for Purchase Order !"))
					else:
						if company.purchase_quatation_template.model_id.model == 'purchase.order':
							res.update({'template_id' : company.purchase_quatation_template.id})
						else:
				...
				if 'template_id' in res:
					if company.invoice_template.model_id.model == 'account.invoice':
						...
						raise Warning(_("You must select email template in general settings only for Invoice ! "))
				else:
					if company.invoice_template.model_id.model == 'account.invoice':
						res.update({'template_id' : company.invoice_template.id})
					else:
