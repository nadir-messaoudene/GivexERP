odoo.define('igt_account_reports.igt_account_reports', function (require) {
"use strict";
    ...
    var accReportsWidget = account_reports.include({

        custom_events: {
            'value_changed': function(ev) {
                 var self = this;
                 self.report_options.partner_account_receivable_ids = ev.data.partner_account_receivable_ids;
        ...
        render_searchview_buttons: function() {
            ...
            // store the current model to add the account domain filter
            var current_model = self.report_model;
            // attach datepicker
            ...
            if (this.report_options.partner) {
                if (!this.M2MFilters) {
                    var fields = {};
                    if ('partner_account_receivable_ids' in this.report_options){
                        var domain = [];
                        if (current_model == 'account.aged.receivable') { domain = [['user_type_id.type', 'in', ['receivable']]]; }
                        else if (current_model == 'account.aged.payable') { domain = [['user_type_id.type', 'in', ['payable']]]; }
                        fields['partner_account_receivable_ids'] = {
                            label: _t('Account'),
                            modelName: 'account.account',
                            value: this.report_options.partner_account_receivable_ids.map(Number),
                            domain: domain,
                        };
                    }
