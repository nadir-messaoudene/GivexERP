...
class PaymentTransaction(models.Model):
    ...
    def _send_payment_request(self):
        ...
        account_invoice_id = self.payment_id.move_id
        trnx_amt = float(convert_curency(acq=self.acquirer_id,amount=self.amount,order_currency=self.currency_id))
    ...
    def _get_tx_from_feedback_data(self, provider, data):
        ...
        tx = self.search([('reference', '=', reference), ('provider', '=', 'monerischeckout')])
        if not tx:
            raise ValidationError(
                "Moneris Checkout: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
    ...
    def _process_feedback_data(self, data):
        ...
        # self.acquirer_reference = response_content.get('acquirer_reference')
