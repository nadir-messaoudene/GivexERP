...
def get_href_params(href):
    href = href.split('?')[1] if '?' in href else href
    href_params = {}
...
class AcquirerMoneris(models.Model):
    ...
    moneris_order_confirmation = fields.Selection(string='Moneris Order Confirmation', selection=[
        ('capture', _('Authorize & capture the amount and conform it'))], default='capture')
    ...
    #             raise UserError(
    #                 _("Card Verification is not supported for this module. Please contact Development Team!"))
    @api.onchange("moneris_transaction_type")
    ...
    def onchange_saving_payment_token(self):
        for rec in self:
            if rec.provider == "monerischeckout":
    ...
    def _get_monerischeckout_environment(self):
        if server_serie == '13.0' or server_serie == '14.0' or server_serie == '15.0':
            environment = 'qa' if self.state == 'test' else 'prod'
        if server_serie == '12.0':
            environment = 'qa' if self.environment == 'test' else 'prod'
        return environment
    ...
    def monerischeckout_s2s_form_process(self, data):
        PaymentToken = False
        acq = self.env['payment.acquirer'].sudo().search(
            [('id', '=', int(data.get('acquirer_id')))])
        ...
        if '/shop/payment' in href and acq.moneris_transaction_type != 'card_verification':
            raise UserError(
                _("You cannot add card using `Purchase` or 'Pre-Authorization`"))
        data_key = ''
    ...
    def monerischeckout_s2s_form_validate(self, data):
        error = dict()
        mandatory_fields = ["ticket_no", "acquirer_id", "partner_id"]
...
class TxMoneris(models.Model):
    ...
    def inline_card_convert(self, data, transaction):
        ...
        transaction['acquirer_reference'] = receipt.get(
            'cc', {}).get('reference_no')
        ...
        if order_id:
            transaction['partner_country_id'] = order_id.partner_id.country_id.id
    ...
    def monerischeckout_gift_convert(self, data, transaction):
        ...
        try:
            ...
            #         }
            #         required_values['acquirer_reference'] = receipt.get('gift', [])[
            #             0].get('reference_no')
    ...
    def monerischeckout_refund_transaction(self, kwargs):
        ...
        if move_id and move_id.payment_reference:
            environment = self.acquirer_id.environment if server_serie == '12.0' else self.acquirer_id.state
            moneris_url = self.acquirer_id._get_s2s_moneris_urls(
                environment)
            ...
            # store_id = self.acquirer_id.moneris_store_id
            # api_token = self.acquirer_id.moneris_api_token
            ...
            processing_country_code = self.acquirer_id.company_id.country_id.code or 'CA'
            status_check = False
            ...
            refund = mpgClasses.mpgHttpsPost(
                ...
                # api_token,
                refund_req)
            refund.postRequest(self.acquirer_id)
            ...
            if receipt.get('ResponseCode') != 'null':
                ...
                if int(receipt.get('ResponseCode')) < 50:
                    ...
                    raise UserError(
                        _("Moneris Refund Failed Response--->" + str(receipt.get('Message'))))

            else:
                raise UserError(
                    _("Moneris Refund Error Response:\n" + str(receipt.get('Message'))))
        else:
            raise UserError(_("Payment TrX reference not found"))

    def update_sale_order(self, order_id):
        sale_lock = self.acquirer_id.moneris_lock_order
    ...
    def monerischeckout_s2s_do_transaction(self, kwargs):
        ...
        false_profiles = self.env['payment.token'].sudo().search(
            [('acquirer_id.provider', '=', 'monerischeckout'), ('moneris_profile', 'in', (False, '')),
        ...
        acquirer_id = form_data.get('acquirer_id') or kwargs.get('acquirer_id')
        ...
        if '/my/payment_method' in href or kwargs.get('card_tokenize') == True:
            ...
            if result == 'a':
                ...
                transaction['acquirer_reference'] = res_json.get(
                    'response', {}).get('receipt', {}).get('reference_no')
                if not self.acquirer_id.moneris_token:
                    self.token_id.write(
        ...
        elif '/shop/payment' in href or '/pay/sale' in href or '/my/orders' in href:
            response = False
            if self.acquirer_id.moneris_token:
                self._monerischeckout_create_token(kwargs)
            ...
            domain += [('state', '=', 'done')]
            domain += [('acquirer_id.provider', '=', 'monerischeckout')]
            ...
            if trnx_amt > 0 and self.acquirer_id.moneris_transaction_type == 'cardverification':
                response = self._monerischeckout_sale_purchase(
                    trnx_amt, paid_amt, order_id)
                if response:
                    return response

            if self.acquirer_id.moneris_transaction_type != 'cardverification':
                res_json = kwargs
                ##########################################################################
                res_json['formData'] = kwargs.get(
                    'formData') if kwargs.get('formData') else {}
        ...
        elif '/my/invoices' in href or '/invoice/pay' in href:
            ...
            tx_domain += [('acquirer_id.provider', '=', 'monerischeckout')]
            ...
            if '/my/orders/' not in href:
                ...
                data_request = {}
                environment = self.acquirer_id._get_monerischeckout_environment()
                url = self.acquirer_id._get_monerischeckout_urls(
                ...
                data_request = {
                    # "store_id": self.acquirer_id.moneris_store_id,
                    # "api_token": self.acquirer_id.moneris_api_token,
                    # "checkout_id": self.acquirer_id.moneris_checkout_id,
                    "ticket": ticket,
                    "environment": environment,
                    "action": "receipt"
                }
                srm = AppPayment(service_name='moneris', service_type='receipt', service_key=self.acquirer.token)
                ...
                if result == 'a':
                    ...
                    transaction = {}
                    transaction['state_message'] = result
                    transaction['acquirer_reference'] = tree.get(
                        'response', {}).get('receipt', {}).get('reference_no')
                    if '/website_payment' in href:
                ...
                # -----New addition
                if model == 'account.move' and self.token_id.moneris_profile and self.acquirer_id.moneris_transaction_type == 'card_verification':
                    ...
                    crypt_type = '7'
                    dynamic_descriptor = ''
                    processing_country_code = self.acquirer_id.company_id.country_id.code
                    environment = self.acquirer_id.environment if server_serie == '12.0' else self.acquirer_id.state
                    moneris_url = self.acquirer_id._get_s2s_moneris_urls(
                        environment)
                    ...
                    purchase = mpgClasses.mpgHttpsPost(
                        moneris_url.get('moneris_server_url'),
                        # self.acquirer_id.moneris_store_id,
                        # self.acquirer_id.moneris_api_token,
                        pur_req)
                    purchase.postRequest(self.acquirer_id)
    ...
    def _monerischeckout_s2s_validate(self, tree):
        ...
        if 'response' in tree:
            if 'receipt' in tree['response']:
                ...
                if self.acquirer_id.moneris_transaction_type != 'card_verification':
                    if tree.get('response', {}).get('success') == 'true':
                        ...
                        if '/shop/payment' in href or '/my/orders' in href:
                            if len(self.sale_order_ids) > 0:
                                ...
                                if self.acquirer_id.moneris_lock_order:
                        ...
                        tx.write({
                            ...
                            'acquirer_reference': receipt.get('ReferenceNum') if receipt.get(
    ...
    def _monerischeckout_invoice_Pay(self, account_invoice_id, trnx_amt, paid_amt, href):
        ...
        if '/shop/payment' in href or '/pay/sale' in href or '/my/orders' in href:
        ...
        type_of = 'res_purchase_cc'
        data_key = self.token_id.moneris_profile
        cust_id = account_invoice_id.partner_id.name + \
        ...
        dynamic_descriptor = ''
        processing_country_code = self.acquirer_id.company_id.country_id.code
        environment = self.acquirer_id.environment if server_serie in [
            '11.0', '12.0'] else self.acquirer_id.state
        moneris_url = self.acquirer_id._get_s2s_moneris_urls(
            environment)
    ...
    def _monerischeckout_create_token(self, kwargs):
        try:
            ...
            token_id = pay_token.create({
                ...
                'acquirer_ref': kwargs['response']['receipt']['cc']['order_no'],
                'name': tokenize['first4last4'],
    ...
    def _monerischeckout_update_token(self, tree, tx, href, token):
        if tree.get('response', {}).get('receipt', {}).get('cc', {}).get('tokenize'):
            ...
            if tokenize.get('success') == 'true' and tokenize.get('status') == '001' and self.acquirer_id.moneris_token:
    ...
    def create_purchase_correction(self, kwargs, transaction_no):
        ...
        environment = self.acquirer_id.state
        moneris_url = self.acquirer_id._get_s2s_moneris_urls(environment)
        ...
        request = correction.postRequest(self.acquirer_id)
        ...
        if tree.get('response', {}).get('receipt', {}):
            ...
            if receipt['Complete'] == 'true' and int(receipt['ResponseCode']) < 50:
                ...
                tx.write({
                    'state': 'error',
                    'state_message': str(receipt.get('Message')).replace("\n", ""),
                    'acquirer_reference': receipt['ReferenceNum'],
                })
                ...
                if '/shop/payment' not in window_href:
                    ...
                    if model == 'account.move':
                        raise UserError(
                            _("Error Processing Moneris Transaction\n<b>Moneris Message:</b> " + str(status)))
                return False
    ...
    def _monerischt_card_convert(self, move_id, data, transaction):
        transaction = {}
        transaction['acquirer_reference'] = data.get('ReferenceNum')
        ...
        #     transaction['date'] = data.get(dateTime, fields.datetime.now())
        transaction['partner_country_id'] = move_id.partner_id.country_id.id
    ...
    def _moneris_fraud_process(self, data):
        try:
            if data.get('response', {}).get('receipt').get('cc', {}).get('fraud', {}):
                ...
                fraud_lines = []
                if fraud.get('avs') and self.acquirer_id.moneris_avs:
                    fraud_line = {}
                ...
                if fraud.get('cvd') and self.acquirer_id.moneris_cvv:
                    fraud_line = {}
                ...
                if fraud.get('3d_secure') and self.acquirer_id.moneris_3d_secure:
                    fraud_line = {}
                ...
                if fraud.get('kount') and self.acquirer_id.moneris_kount:
                    fraud_line = {}
...
class PaymentToken(models.Model):
    ...
    provider = fields.Selection(
        string='Provider', related='acquirer_id.provider', readonly=False)
    # change
    save_token = fields.Boolean(
        string='Save Cards', related='acquirer_id.allow_tokenization', readonly=False)

    def unlink(self):
        if self.acquirer_id.provider == 'monerischeckout':
            if not self.acquirer_id.allow_token_delete:
