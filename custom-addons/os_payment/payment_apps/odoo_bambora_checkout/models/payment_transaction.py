...
class PaymentTransaction(models.Model):
    ...
    def bamborachk_s2s_do_transaction(self, **data):
        self.ensure_one()
        # tran_profile = None
        false_profiles = self.env['payment.token'].sudo().search([('acquirer_id.provider', '=', 'bamborachk'), (
        ...
        if acq:
            ...
            if self.token_id:
                ...
                partner_name = self.token_id.bamborachk_profile_name or partner_name
        ...
        if 'approved' in tree:
            if tree.get("approved") == '1':
                if len(self.sale_order_ids) > 0:
                    ...
                    self.sale_order_ids[0].with_user(SUPERUSER_ID).sudo().action_confirm()
                    self.sale_order_ids[0].sudo()._send_order_confirmation_mail()
                _logger.info("TRANSACTION APPROVED")
                transaction = {}
                transaction['acquirer_reference'] = str(tree.get('authorizing_merchant_id')) + "/" + tree.get(
                ...
                if len(self.sale_order_ids) > 0:
                    ...
                    transaction['partner_country_id'] = self.sale_order_ids[0].partner_id.country_id.id or self.acquirer_id.company_id.id
                ...
                tx.with_company(tx.company_id)._set_done()
                # tx.with_company(tx.company_id)._reconcile_after_done()
                return True
            else:
                _logger.warning("TRANSACTION DECLINED")
    ...
    def _send_payment_request(self):
        ...
        if not self.token_id:
            raise UserError("Bambora Checkout: " + _("The transaction is not linked to a token."))
        ...
        self._handle_feedback_data('bamborachk', notification_data)
...
class PaymentToken(models.Model):
    ...
    bamborachk_profile_name = fields.Char()
    bamborachk_token = fields.Char()
    bamborachk_token_type = fields.Selection(
        string='Token Type',
        selection=[('temporary', 'Temporary'), ('permanent', 'Permanent')])
    acquirer = fields.Selection(
        string='acquirer', related='acquirer_id.provider', readonly=False)
    ...
    def _get_tx_from_feedback_data(self, provider, data):
        ...
        if not tx:
            raise ValidationError(
                "Bambora Checkout: " + _("No transaction found matching reference %s.", reference)
            )
        return tx
