...
class Acquirerbambora(models.Model):
    ...
    provider = fields.Selection(selection_add=[('bamborachk', 'Bambora Checkout')], 
        ondelete={'bamborachk': 'set default'})
    bamborachk_transaction_type = fields.Selection(
        string=_("Transaction Type"),
        selection=[('purchase', _('Purchase'))], default='purchase')
    ...
    def _get_feature_support(self):
        res = super(Acquirerbambora, self)._get_feature_support()
    ...
    def _compute_view_configuration_fields(self):
        ...
        self.filtered(lambda p: p.provider == 'bamborachk').update({
    ...
    def bamborachk_s2s_form_process(self, data):
        ...
        values = {
            ...
            'acquirer_ref': 'bamborachk',
            'partner_id': data.get('partner_id'),
            'name': token_name
        }
        acq = self.env['payment.acquirer'].sudo().browse(int(data.get('acquirer_id')))
        ...
        if 'my/payment_method' in request.params.get("window_href") or optional_save_token_request:
            ...
            if response['error'] == None or 'errors_message' not in response:
                pro_res = response
                if pro_res.get("code") == 1:
                    ...
                    raise ValidationError(_(response.get('message')))
            else:
    ...
    def bamborachk_s2s_form_validate(self, data):
        ...
        error = dict()
        mandatory_fields = ["acquirer_id", "acquirer_state",  "bamborachk_merchant_id", "bamborachk_payment_api", "bamborachk_profile_api", "order_name", "charge_total",
        ...
        if 'my/payment_method' in request.params.get("window_href") or '/my/orders' in request.params.get("window_href"):
            mandatory_fields = ["acquirer_id", "acquirer_state",  "bamborachk_merchant_id", "bamborachk_payment_api",
