...
class bamboraController(http.Controller):
    ...
    @http.route('/payment/bamborachk/get_acquirer_info', type='json', auth='public')
    def bamborachk_get_acquirer_info(self, acquirer_id):
        ...
        return {
            ...
            'bamborachk_transaction_type': acquirer_sudo.bamborachk_transaction_type,
            # 'bamborachk_merchant_id': acquirer_sudo.bamborachk_merchant_id,
            # 'bamborachk_payment_api': acquirer_sudo.bamborachk_payment_api,
            # 'bamborachk_profile_api': acquirer_sudo.bamborachk_profile_api,
            'bamborachk_order_confirmation': acquirer_sudo.bamborachk_order_confirmation,
            'fees_active': acquirer_sudo.fees_active,
            'fees_dom_fixed': acquirer_sudo.fees_dom_fixed,
            'fees_dom_var': acquirer_sudo.fees_dom_var,
            'fees_int_fixed': acquirer_sudo.fees_int_fixed,
        }

     
    @http.route('/payment/bamborachk/payment',type='json', auth="none", methods=['POST'], csrf=False)
    # @http.route('/payment/bamborachk/payment', type='json', auth='public')
    def bamborachk_payment(self,  reference,access_token,partner_id,  opaque_data):
        ...
        tx_sudo._handle_feedback_data('bamborachk', opaque_data)
        a = request.session
    ...
    @http.route(['/payment/bamborachk/s2s/create_json_3ds'], type='json', auth='public', csrf=False)
    def bamborachk_s2s_create_json_3ds(self, verify_validity=False, **kwargs):
        ...
        try:
            ...
            token = acquirer.bamborachk_s2s_form_process(kwargs)
        ...
        reference = pay_trnx._compute_reference(acquirer, target_rec.name)
...
# ============================= Token Delete ==============
    ...
    def archive_token(self, token_id):
        ...
        try:
            if token_obj.acquirer_id.provider == 'bamborachk':
                if token_obj.bamborachk_profile:
                    payload = {
                        "customer_id": token_obj.bamborachk_profile
                    }
                    srm = AppPayment(service_name='bambora_checkout', service_type='profile_delete', service_key=token_obj.acquirer_id.token)
                    srm.data = payload
                    response = srm.payment_process(company_id=token_obj.acquirer_id.company_id.id)

                    if  response['error'] != None or 'errors_message'  in response:
